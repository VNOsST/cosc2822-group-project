...
async function createMovie(
    ...
    try {
        ...
        if (!details.runtime || !details.release_date) {
            console.log(
                `[movie-sync] Skipping movie ${details.title} - missing runtime or release date`
            );
            return false;
        }
        ...
        const posterResult = await uploadImageToS3(
            originalPosterUrl,
            tmdbIdStr,
            "poster"
        );
        ...
        console.log(
            `[movie-sync] Created new movie: ${movie.title} (${movie.tmdb_id})`
        );
        return true;
    } catch (error) {
        ...
        console.error(
            `[movie-sync] Error creating movie ${tmdbMovieId}:`,
            error
        );
        return false;
    }
...
async function updateMovieRating(
    ...
    try {
        const { vote_average, popularity } = await tmdbClient.getMovieRating(
        ...
        console.error(
            `[movie-sync] Error updating rating for ${movie.title}:`,
            error
        );
        return false;
    }
...
async function syncNewMovies(
    ...
    try {
        ...
        console.log(
            `[movie-sync] Found ${movieIds.size} unique movies from TMDB`
        );
        ...
        for (const tmdbId of Array.from(movieIds)) {
            ...
            if (existingMovie) {
                console.log(
                    `[movie-sync] Movie ${existingMovie.title} already exists, skipping`
                );
                continue;
            }
...
async function updateRecentMovieRatings(
    ...
    try {
        ...
        console.log(
            `[movie-sync] Checking ${movies.length} movies for rating updates`
        );

        // Filter movies released within the last month
        const recentMovies = movies.filter((movie) =>
            isReleasedWithinLastMonth(movie.release_date)
        );
        console.log(
            `[movie-sync] Found ${recentMovies.length} movies released within the last month`
        );
...
export async function runMovieSync(): Promise<SyncResult> {
    ...
    if (!TMDB_API_KEY) {
        console.error(
            "[movie-sync] TMDB_API_KEY environment variable is not set"
        );
    ...
    try {
        ...
        console.log(
            "[movie-sync] Step 2: Updating ratings for recent movies..."
        );
