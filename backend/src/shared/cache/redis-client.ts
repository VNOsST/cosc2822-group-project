...
const redisEndpoint =
    ...
    (isLocalDevelopment ? "localhost" : undefined);
...
export async function getRedisClient(): Promise<RedisClientType> {
    ...
    if (!redisEndpoint) {
        throw new Error(
            "Redis endpoint not configured. Set REDIS_ENDPOINT environment variable."
        );
    }
        ...
        socket: {
            ...
            reconnectStrategy: (retries) => {
                if (retries > 2) {
                    console.error(
                        "[redis]",
                        `Max reconnection attempts reached after ${retries} retries`
                    );
                ...
                console.log(
                    "[redis]",
                    `Reconnecting in ${delay}ms (attempt ${retries + 1})`
                );
                return delay;
            },
        },
...
export async function unlockSeats(
    ...
    const results = await Promise.all(
        seatIds.map(async (seatId) => {
            const success = await unlockSeat(
                showtimeId,
                seatId,
                userId,
                lockId
 ...
 */
export async function getShowtimeLocks(
    showtimeId: string
): Promise<Map<string, SeatLockData>> {
    const client = await getRedisClient();
...
export async function extendLocks(
    ...
    const results = await Promise.all(
        seatIds.map(async (seatId) => {
            const newExpiry = await extendLock(
                showtimeId,
                seatId,
                userId,
                lockId
 ...
 */
export async function releaseLocksForBooking(
    showtimeId: string,
    seatIds: string[]
): Promise<void> {
    const client = await getRedisClient();
