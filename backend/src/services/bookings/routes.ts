        ...
        const bookingsWithDetails = await Promise.all(
            bookings.map(async (booking) => {
                ...
                try {
                    ...
                    console.error(
                        `Error fetching movie ${booking.movie_id}:`,
                        error
                    );
                }
                ...
                try {
                    ...
                    showtime = showtimeResult.Items?.[0] as
                        | Showtime
                        | undefined;
                    ...
                    console.error(
                        `Error fetching showtime ${booking.showtime_id}:`,
                        error
                    );
                }
    ...
    try {
        ...
        if (data.lock_id) {
            try {
                ...
                console.warn(
                    "[bookings]",
                    "Lock validation skipped (Redis unavailable):",
                    lockError
                );
        ...
        if (data.lock_id) {
            try {
                ...
                console.warn(
                    "[bookings]",
                    "Failed to release locks (non-critical):",
                    lockError
                );
    ...
    try {
        ...
        bookings.forEach((b) => {
            if (b.status === "confirmed") {
                movieBookings[b.movie_id] =
                    (movieBookings[b.movie_id] || 0) + 1;
            }
        });
        ...
        return c.json({
            ...
            data: {
                ...
                average_booking_value:
                    Math.round(averageBookingValue * 100) / 100,
        ...
        console.error(
            "[bookings]",
            "Error fetching booking statistics:",
            error
        );
        return c.json(
