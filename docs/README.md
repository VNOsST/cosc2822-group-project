# Documentation Index

Welcome to the CineCloud project documentation. This directory contains all technical and project documentation.

## 📚 Documentation Structure

### Getting Started

-   [Quick Start Guide](./guides/QUICK_START.md) - Get up and running quickly
-   [Installation](./guides/INSTALLATION.md) - Detailed installation instructions
-   [Local Development](./guides/LOCAL_DEVELOPMENT.md) - Setting up your development environment

### Architecture & Design

-   [System Architecture](./architecture/SYSTEM_ARCHITECTURE.md) - Overall system design
-   [Microservices Architecture](./architecture/MICROSERVICES.md) - Backend microservices design
-   [Folder Structure](./architecture/FOLDER_STRUCTURE.md) - Code organization guide
-   [Database Schema](./architecture/DATABASE_SCHEMA.md) - DynamoDB table structures
-   [API Documentation](./api/API_REFERENCE.md) - Complete API reference

### Development

-   [Branching Strategy](./development/BRANCHING_STRATEGY.md) - Git workflow and branching
-   [Commit Convention](./development/COMMIT_CONVENTION.md) - Commit message standards
-   [Code Style Guide](./development/CODE_STYLE.md) - Coding standards and best practices
-   [Testing Guide](./development/TESTING.md) - Testing strategies and examples

### Deployment

-   [Backend Deployment](./deployment/BACKEND_DEPLOYMENT.md) - AWS Lambda deployment
-   [Frontend Deployment](./deployment/FRONTEND_DEPLOYMENT.md) - Frontend deployment
-   [Database Setup](./deployment/DATABASE_SETUP.md) - DynamoDB setup and management
-   [Docker Setup](./deployment/DOCKER_SETUP.md) - Docker and Docker Compose usage

### Project Information

-   [Project Proposal](./project/PROJECT_PROPOSAL.md) - Original project proposal
-   [Team Information](./project/TEAM.md) - Team members and roles
-   [Contributing](./project/CONTRIBUTING.md) - How to contribute

## 🔍 Quick Links

### For Developers

-   [Getting Started](./guides/QUICK_START.md)
-   [API Reference](./api/API_REFERENCE.md)
-   [Commit Convention](./development/COMMIT_CONVENTION.md)

### For DevOps

-   [Backend Deployment](./deployment/BACKEND_DEPLOYMENT.md)
-   [Docker Setup](./deployment/DOCKER_SETUP.md)

### For Architects

-   [System Architecture](./architecture/SYSTEM_ARCHITECTURE.md)
-   [Microservices Architecture](./architecture/MICROSERVICES.md)
-   [Folder Structure](./architecture/FOLDER_STRUCTURE.md)
-   [Database Schema](./architecture/DATABASE_SCHEMA.md)

## 📝 Document Conventions

-   All documents are written in Markdown
-   Code examples include language identifiers
-   Screenshots and diagrams in `./assets/` directory
-   Keep documents focused and modular

## 🆘 Need Help?

-   Check the [Quick Start Guide](./guides/QUICK_START.md) first
-   Review the [API Reference](./api/API_REFERENCE.md) for endpoint details
-   See [Contributing Guide](./project/CONTRIBUTING.md) for contribution guidelines
