...
These challenges are not hypothetical \- they have been highlighted on a global scale during major blockbuster releases. For example, during the pre-sale launch of *Avengers: Endgame*, demand was so overwhelming that multiple ticketing platforms crashed, leaving customers unable to complete purchases and creating widespread dissatisfaction \[1,2\]. A similar scenario unfolded with *Spider-Man: No Way Home*, where unprecedented traffic again caused major box office websites and apps to fail during peak demand \[3\].
