...
import { AlertCircle, Bell, CheckCircle2, Loader2, Mail, Send, Trash2 } from 'lucide-react'
...
function AdminNotificationsPage() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Subscription request sent! Please check your email to confirm.')
      setEmail('')
    } catch (error) {
  ...
  const isSubscribed = subscriptions?.some(s => s.email === user?.email && s.status === 'confirmed')
  const isPending = subscriptions?.some(s => s.email === user?.email && s.status === 'pending')

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold tracking-tight text-foreground">Admin Notifications</h2>
        <p className="text-muted-foreground">
          Manage email subscriptions for system alerts (bookings, cancellations, etc.)
        </p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Add an email address to receive real-time admin notifications via SNS.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {user && !isSubscribed && !isPending && (
              <p className="mt-4 text-sm text-muted-foreground">
                You are not currently subscribed to alerts with your account email ({user.email}).
              </p>
            )}
            {isPending && (
              <div className="mt-4 flex items-center gap-2 rounded-lg bg-amber-500/10 p-3 text-sm text-amber-600 dark:text-amber-400">
                ...
                <span>Subscription pending confirmation for {user?.email}. Check your inbox!</span>
              </div>
            )}
            {isSubscribed && (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Send a test message to all confirmed subscribers to verify the system.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button 
              ...
              <Table>
                ...
                <TableBody>
                  {subscriptions?.map((sub) => (
                    <TableRow key={sub.subscriptionArn} className="border-border">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            The following events will trigger an email notification to all confirmed subscribers.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <ul className="grid gap-4 md:grid-cols-2">
            {[
              { title: 'New Booking', desc: 'Sent when a user confirms a new ticket reservation.' },
              { title: 'Booking Cancelled', desc: 'Sent when a user cancels their booking within the 6h window.' },
              { title: 'User Registration', desc: 'Sent when a new user confirms their email address.' },
              { title: 'Low Seat Availability', desc: 'Triggered when a showtime reaches 80% capacity.' },
            ].map((item) => (
              <li key={item.title} className="flex gap-3 rounded-lg border border-border p-4">
