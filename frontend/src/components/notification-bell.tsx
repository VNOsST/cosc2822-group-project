...
import { useDeleteNotification, useMarkNotificationAsRead, useNotifications } from '@/hooks/use-notifications-api'
...
export function NotificationBell() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-80 p-0 bg-slate-900 border-white/10 text-slate-100 shadow-2xl">
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-3">
                      <div className={`mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${notification.type === 'rating_prompt' ? 'bg-amber-500/10 text-amber-500' : 'bg-blue-500/10 text-blue-500'}`}>
                        {notification.type === 'rating_prompt' ? <MessageSquare className="h-4 w-4" /> : <Bell className="h-4 w-4" />}
                      </div>
                  ...
                  {notification.type === 'rating_prompt' && notification.metadata?.movie_id && (
                    <Button asChild variant="link" className="h-auto p-0 text-amber-500 text-xs justify-start hover:text-amber-400">
                      <Link to="/public/movies/$id" params={{ id: notification.metadata.movie_id }}>
                        Leave a review
                      </Link>
                    </Button>
                  )}
